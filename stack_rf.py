...
if __name__ == '__main__':
    ...
    registered_rf_params = {"max_features": None, "max_depth": 20,
                        "max_leaf_nodes": 2**14, "min_samples_split": 4,
                        "min_samples_leaf": 2, "n_estimators": 1000, "n_jobs": 4}

    casual_rf_params = registered_rf_params
    # casual_rf_params = {"max_features": None, "max_depth": 20,
    ...
    search_space_regist = {"max_features": [None], "max_depth": [20],
                    "max_leaf_nodes": [2**14], "min_samples_split": [4],
                    "min_samples_leaf": [2], "n_estimators": [1000]}

    search_space_casual = search_space_regist
    # search_space_casual = {"max_features": [None], "max_depth": [20],
    #                 "max_leaf_nodes": [2**10], "min_samples_split": [8],
    #                 "min_samples_leaf": [4], "n_estimators": [500]}
    ...
    if do_optimize:
        ...
        regist_grid_search = sklearn.grid_search.GridSearchCV(ens.RandomForestRegressor(
                            ),
                            search_space_regist, cv=cv_gs, scoring="mean_squared_error",
                            refit=False, n_jobs=4, pre_dispatch="2*n_jobs", verbose=3)

        casual_grid_search = sklearn.grid_search.GridSearchCV(ens.RandomForestRegressor(
                            ),
    ...
    gb_trees_regist_regr = ens.RandomForestRegressor(**registered_rf_params)
    gb_trees_casual_regr = ens.RandomForestRegressor(**casual_rf_params)

    print("training using {0:} samples with {1:} features each".format(*train_regist_X.shape))
    print("training registered rf using classifier:")
    print(gb_trees_regist_regr)
    gb_trees_regist_regr.fit(train_regist_X, train_regist_y)
    print("training casual rf using classifier:")
    ...
    # predict the log count diff using rf
    print("predicting the log counts and counts on the training set with rf")
    train_set_df["log_registered_rf_pred"] = gb_trees_regist_regr.predict(train_regist_X)
    train_set_df["log_casual_rf_pred"] = gb_trees_casual_regr.predict(train_casual_X)

    train_set_df["registered_rf_pred"] = np.exp(train_set_df["log_registered_rf_pred"]) - 1
    train_set_df["casual_rf_pred"] = np.exp(train_set_df["log_casual_rf_pred"]) - 1

    train_set_df["count_rf_pred"] = train_set_df["registered_rf_pred"] + train_set_df["casual_rf_pred"]
    ...
    # predict the log count diff using rf, calculate count
    print("predicting the log differences, log counts, and counts on the test set with rf")
    test_set_df["log_registered_rf_pred"] = gb_trees_regist_regr.predict(test_regist_X)
    test_set_df["log_casual_rf_pred"] = gb_trees_casual_regr.predict(test_casual_X)

    test_set_df["registered_rf_pred"] = np.exp(test_set_df["log_registered_rf_pred"]) - 1
    test_set_df["casual_rf_pred"] = np.exp(test_set_df["log_casual_rf_pred"]) - 1

    test_set_df["count_rf_pred"] = test_set_df["registered_rf_pred"] + test_set_df["casual_rf_pred"]
    test_set_df["count_rf_pred"] = test_set_df["count_rf_pred"].apply(np.round).apply(int)

    # save data and predicted trend to a new csv
    if not os.path.exists("stack_rf_submit.csv") or overwrite:
        test_set_df.to_csv("stack_rf_submit.csv", columns=["count_rf_pred"], header=["count"])
