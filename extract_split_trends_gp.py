...
train_set_clean_df["casual"] = by_month_gp["casual"].apply(pd.Series.interpolate, method="time")
train_set_clean_df["registered"] = by_month_gp["registered"].apply(pd.Series.interpolate, method="time")
...
# backfill missing workingday and holiday information for previously interpolated times
train_set_clean_df["workingday"] = by_month_gp["workingday"].fillna(method="bfill")
train_set_clean_df["holiday_clean"] = by_month_gp["holiday"].fillna(method="bfill")
...
if fix_workdays:
    ...
    train_set_clean_df["holiday_clean"][pd.datetime(2011, 4, 15, 0):pd.datetime(2011, 4, 15, 23)] = 0
    train_set_clean_df["holiday_clean"][pd.datetime(2012, 4, 16, 0):pd.datetime(2012, 4, 16, 23)] = 0

# calculate the sum for each day
daily_sum_df = pd.DataFrame(train_set_clean_df["count"].resample("D", how="sum").dropna())
daily_sum_df["casual"] = train_set_clean_df["casual"].resample("D", how="sum").dropna()
daily_sum_df["registered"] = train_set_clean_df["registered"].resample("D", how="sum").dropna()
daily_sum_df["workingday_clean"] = train_set_clean_df["workingday_clean"].resample("D", how="mean")
daily_sum_df["workingday_clean"] = daily_sum_df["workingday_clean"].apply(int)
daily_sum_df["holiday_clean"] = train_set_clean_df["holiday_clean"].resample("D", how="mean")
daily_sum_df["holiday_clean"] = daily_sum_df["holiday_clean"].apply(int)

daily_sum_df.groupby(["workingday_clean"])["casual"].plot(style="o-")
daily_sum_df.groupby(["workingday_clean"])["registered"].plot(style="o-")
plt.legend()
# train_set_clean_df.groupby(["workingday_clean"])["casual"].plot()
# train_set_clean_df.groupby(["workingday_clean"])["registered"].plot()
plt.show()
qqqqqq
