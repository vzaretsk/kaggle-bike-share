...
if __name__ == '__main__':
    ...
    # parameters for the hourly bagged adaboost regression
    registered_bag_params = {"n_estimators": 20, "max_samples": 0.7, "bootstrap": True}
    registered_ada_params = {"loss": "linear", "learning_rate": 0.1, "n_estimators": 50}
    registered_tree_params = {"max_depth": None, "max_features": None, "max_leaf_nodes": 2**10,
                              "min_samples_split": 8, "min_samples_leaf": 4}

    casual_bag_params = registered_bag_params
    casual_ada_params = registered_ada_params
    casual_tree_params = registered_tree_params
    # casual_bag_params = {"n_estimators": 10, "max_samples": 1.0, "bootstrap": True}
    ...
    bag_p = ""
    ada_p = "base_estimator__"
    tree_p = "base_estimator__base_estimator__"
    # loss may be "linear", "square", "exponential"
    # parameters for the gradient boosted trees hourly fit cross validation
    search_space_regist = {bag_p+"n_estimators": [20], bag_p+"max_samples": [0.7], bag_p+"bootstrap": [True],
                           ada_p+"loss": ["linear"], ada_p+"learning_rate": [0.1], ada_p+"n_estimators": [50],
                           tree_p+"max_depth": [None], tree_p+"max_features": [None], tree_p+"max_leaf_nodes": [2**10],
                           tree_p+"min_samples_split": [8], tree_p+"min_samples_leaf": [4]}

    # search_space_casual = search_space_regist
    search_space_casual = {bag_p+"n_estimators": [20], bag_p+"max_samples": [0.7], bag_p+"bootstrap": [True],
                           ada_p+"loss": ["linear"], ada_p+"learning_rate": [0.1], ada_p+"n_estimators": [50],
                           tree_p+"max_depth": [None], tree_p+"max_features": [None], tree_p+"max_leaf_nodes": [2**10],
                           tree_p+"min_samples_split": [8], tree_p+"min_samples_leaf": [4]}
    ...
    if do_optimize:
        ...
        regist_grid_search = sklearn.grid_search.GridSearchCV(ens.BaggingRegressor(
                                        base_estimator=ens.AdaBoostRegressor(
        ...
        casual_grid_search = sklearn.grid_search.GridSearchCV(ens.BaggingRegressor(
                                        base_estimator=ens.AdaBoostRegressor(
                                            base_estimator=tree.DecisionTreeRegressor())),
    ...
    ada_regist_regr  = ens.BaggingRegressor(base_estimator=
                           ens.AdaBoostRegressor(base_estimator=tree.DecisionTreeRegressor(**registered_tree_params),
                                                 **registered_ada_params),
                                            n_jobs=3, **registered_bag_params)
    ada_casual_regr = ens.BaggingRegressor(base_estimator=
                           ens.AdaBoostRegressor(base_estimator=tree.DecisionTreeRegressor(**registered_tree_params),
                                                 **registered_ada_params),
                                            n_jobs=3, **registered_bag_params)

    print("training using {0:} samples with {1:} features each".format(*train_X.shape))
    print("training registered bagged adaboost using classifier:")
    print(ada_regist_regr)
    ada_regist_regr.fit(train_X, train_regist_y)
    print("training casual bagged adaboost using classifier:")
    ...
    print("predicting the log differences, log counts, and counts on the training set with bagged adaboost")
    train_set_df["log_registered_ada_diff"] = ada_regist_regr.predict(train_X)
    train_set_df["log_casual_ada_diff"] = ada_casual_regr.predict(train_X)

    train_set_df["log_registered_ada_pred"] = train_set_df["log_registered_ada_diff"] + \
                                              train_set_df["log_registered_gp_pred"]
    train_set_df["log_casual_ada_pred"] = train_set_df["log_casual_ada_diff"] + \
                                              train_set_df["log_casual_gp_pred"]

    train_set_df["registered_ada_pred"] = np.exp(train_set_df["log_registered_ada_pred"]) - 1
    train_set_df["casual_ada_pred"] = np.exp(train_set_df["log_casual_ada_pred"]) - 1

    train_set_df["count_ada_pred"] = train_set_df["registered_ada_pred"] + train_set_df["casual_ada_pred"]

    train_set_df["log_count_ada_pred"] = np.log(train_set_df["count_ada_pred"]+1)

    score = np.sqrt(((train_set_df["log_count"] -
                      train_set_df["log_count_ada_pred"])**2).mean())
    print("training data score (RMSE): {:0.5f}".format(score))

    # bagging regressor has no feature_importances_
    ...
    (train_set_df["log_count"] - train_set_df["log_count_ada_pred"]).plot(style=".-k")
    fig_num += 1
    ...
    test_set_df["log_registered_ada_diff"] = ada_regist_regr.predict(test_X)
    test_set_df["log_casual_ada_diff"] = ada_casual_regr.predict(test_X)

    test_set_df["log_registered_ada_pred"] = test_set_df["log_registered_ada_diff"] + \
                                              test_set_df["log_registered_gp_pred"]
    test_set_df["log_casual_ada_pred"] = test_set_df["log_casual_ada_diff"] + \
                                              test_set_df["log_casual_gp_pred"]

    test_set_df["registered_ada_pred"] = np.exp(test_set_df["log_registered_ada_pred"]) - 1
    test_set_df["casual_ada_pred"] = np.exp(test_set_df["log_casual_ada_pred"]) - 1

    test_set_df["count_ada_pred"] = test_set_df["registered_ada_pred"] + test_set_df["casual_ada_pred"]
    test_set_df["count_ada_pred"] = test_set_df["count_ada_pred"].apply(np.round).apply(int)

    # save the predicted log_registered_ada_diff, log_casual_ada_diff
    pred_df = pd.concat([train_set_df[["log_registered_ada_pred", "log_casual_ada_pred"]],
                                    test_set_df[["log_registered_ada_pred", "log_casual_ada_pred"]]])
    pred_df.sort_index(inplace=True)

    if not os.path.exists("ada_bag_hourly_pred.csv") or overwrite:
        pred_df.to_csv("ada_bag_hourly_pred.csv")

    # save data and predicted trend to a new csv
    if not os.path.exists("ada_bag_hourly_submit.csv") or overwrite:
        test_set_df.to_csv("ada_bag_hourly_submit.csv", columns=["count_ada_pred"], header=["count"])
