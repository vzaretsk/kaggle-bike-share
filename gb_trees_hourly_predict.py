...
if __name__ == '__main__':
    ...
    registered_gbt_params = {"loss": "ls", "learning_rate": 0.01, "max_depth": 10,
    ...
    search_space_regist = {"loss": ["ls"], "learning_rate": [0.01],
    ...
    gb_trees_regist_regr.fit(train_X, train_regist_y)
    print("training casual gb trees using classifier:")
    print(gb_trees_casual_regr)
    gb_trees_casual_regr.fit(train_X, train_casual_y)
    ...
    print("predicting the log differences, log counts, and counts on the training set with gb trees")
    train_set_df["log_registered_gb_diff"] = gb_trees_regist_regr.predict(train_X)
    train_set_df["log_casual_gb_diff"] = gb_trees_casual_regr.predict(train_X)
    ...
    feature_import_regist_lst = list(zip(features_lst, gb_trees_regist_regr.feature_importances_))
    feature_import_casual_lst = list(zip(features_lst, gb_trees_casual_regr.feature_importances_))
    ...
    test_set_df["log_registered_gb_diff"] = gb_trees_regist_regr.predict(test_X)
    test_set_df["log_casual_gb_diff"] = gb_trees_casual_regr.predict(test_X)
    ...
    if not os.path.exists("gb_trees_hourly_pred.csv") or overwrite:
        pred_df.to_csv("gb_trees_hourly_pred.csv")

    # save data and predicted trend to a new csv
    if not os.path.exists("gb_trees_hourly_submit.csv") or overwrite:
        test_set_df.to_csv("gb_trees_hourly_submit.csv", columns=["count_gb_pred"], header=["count"])
